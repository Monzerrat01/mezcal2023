...
%token NAME COLON RIGHT_ARROW LEFT_CURLY_BRACE RIGHT_CURLY_BRACE SEMICOLON LEFT_PARENTHESIS RIGHT_PARENTHESIS SINGLECOMMENT
	MULTILINECOMMENT SHOW QUOTES CHARACTERS_BLOCK INT INTEGER_VALUE DECIMAL_VALUE LOAD STDIN DOLLAR_SIGN INC DEC DECIMAL
	BLN SET TRUE FALSE NTOL QUESTION_MARK CORCH_IZQ CORCH_DERECHO EQ LE LT GT GE NE ELSE COMA STR DOBLE_COLON WHILE ANSWER ADD
	DOB_SLASH_IN LES SLASH SEARCH
...
bifurcation:
	...
	|
	ELSE LEFT_CURLY_BRACE statement RIGHT_CURLY_BRACE { $$ = "else{\n" + $3 + "\n}\n";}
	|
...
logical_eval:
	DOLLAR_SIGN name CORCH_IZQ DOLLAR_SIGN name CORCH_DERECHO comp_operator DOLLAR_SIGN name { $$ = $2 + "[" + $5 + "]" + $7 + $9; }
	|
	DOLLAR_SIGN name CORCH_IZQ DOLLAR_SIGN name CORCH_DERECHO comp_operator DOLLAR_SIGN name CORCH_IZQ DOLLAR_SIGN name CORCH_DERECHO { $$ = $2 + "[" + $5 + "]" + $7 + $9 + "[" + $12 + "]"; }
	|
...
ids:
	...
	;
std_output:
	SEARCH COLON characters_block { $$ = "std::cout << " + $3 + ";"; }
	|
